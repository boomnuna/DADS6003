# AGENTS.md — Credit Card Clustering (Round 10: Cap instead of Drop)

## Goal
Replace IQR outlier DROPPING with IQR CAPPING to keep all 8,950 rows.
Compare Silhouette score with Round 9 (0.7483, 69% data) to see if
capping gives a more honest and trustworthy result.

## Key difference from all previous rounds
- Previous rounds: DROP rows where any feature is outside IQR bounds → lost 31% of data
- This round: CLIP values to IQR bounds → keep 100% of data, just limit extreme values

## Fixed params (best from Round 8/9)
- UMAP: n_neighbors=50, min_dist=0.1, metric=euclidean, n_components=7
- DBSCAN: min_samples=3, eps=2.0
- All preprocessing: log transform + RobustScaler + correlation feature selection

---

## Baseline to compare

| Approach | Rows used | Silhouette | Honest? |
|----------|-----------|------------|---------|
| Drop (R9) | 6,157 (69%) | 0.7483 | ⚠️ Partial |
| **Cap (R10)** | **8,950 (100%)** | **?** | **✅ Full** |

---

## Data File
- `CC_GENERAL.csv` — 8,950 rows
...
## Step 1 — Load and Preprocess with CAPPING (not dropping)
...
# NEW: CAP outliers instead of DROP
# clip() replaces values outside bounds with the boundary value
# No rows are removed — all 8,950 customers stay
...
## Step 2 — Log Transform + Feature Engineering + Drop Correlated
...
# Fixed best params from Round 8/9
N_NEIGHBORS  = 50
MIN_DIST     = 0.1
METRIC       = "euclidean"
N_COMPONENTS = 7
...
plt.close()
print("Saved → r10_cap_vs_drop.png")
```
...
| File | Description |
|------|-------------|
| `r10_cap_vs_drop.png` | Cluster scatter + Drop vs Cap bar chart |
| `clustering_results_r10.csv` | All 8,950 rows + cluster labels (no -99) |

## Expected Console Output
1. Capping stats — how many values capped per feature
2. DBSCAN results (clusters, noise%, Silhouette, Davies-Bouldin)
3. Drop vs Cap honest comparison table
4. Cluster profile (original scale)
5. Token usage

---

## Constraints
- Do NOT drop any rows — all 8,950 must be in df_final
- Use clip() for capping, not manual replacement
- UMAP and DBSCAN params are FIXED — do not tune
- Profile clusters using capped values (df_capped), not log-transformed
- Output CSV must have no -99 values
- Print honest comparison table showing both Silhouette AND coverage
