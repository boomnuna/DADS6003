...
print(f"Fraud rate train: {train['Class'].mean():.6f}, test: {test['Class'].mean():.6f}")
...
neg = (y_train == 0).sum()
pos = (y_train == 1).sum()
print(f"Features: {X_train.shape[1]}, Neg: {neg}, Pos: {pos}, Ratio: {neg/pos:.1f}")

# ============================================================
# STEP 3 — Tune XGBoost with Optuna
# ============================================================
print("\n" + "=" * 60)
print("STEP 3: Tune XGBoost with Optuna (100 trials)")
print("=" * 60)
...
# ============================================================
# STEP 4 — Re-tune LightGBM with New Features
# ============================================================
print("\n" + "=" * 60)
print("STEP 4: Tune LightGBM with Optuna (100 trials)")
print("=" * 60)
...
print(f"Best LightGBM AUC: {lgb_study.best_value:.6f}")
...
# ============================================================
print("\n" + "=" * 60)
print("STEP 5: Optimized Ensemble with Weight Search (200 trials)")
print("=" * 60)
...
# ============================================================
print("\n" + "=" * 60)
print("STEP 6: Evaluate and Save All Results")
print("=" * 60)
...
evaluate_and_save(
    ...
    y_test,
    preds_lgb,
    notes=f"v2 features, 100 trials. Best params: {lgb_study.best_params}",
)
evaluate_and_save(
    "XGBoost_Optuna_v2",
    y_test,
    preds_xgb,
    notes=f"v2 features, 100 trials. Best params: {xgb_study.best_params}",
)
evaluate_and_save(
    "Ensemble_v2_OptWeights",
    y_test,
    final_blend,
    notes=f"Optuna weight search. w_lgb={best_w['w_lgb']:.3f}",
)

# Save best predictions
pd.DataFrame({"y_true": y_test, "y_prob": final_blend}).to_csv(
...
# Check if we need a 3rd model
best_auc = roc_auc_score(y_test, final_blend)
print(f"\n{'=' * 60}")
print(f"FINAL ENSEMBLE ROC-AUC: {best_auc:.6f}")
if best_auc < 0.977:
    print("WARNING: AUC below 0.977 — consider adding CatBoost to ensemble")
else:
    print(f"Goal achieved! Beat previous best of 0.977484 ✓")
